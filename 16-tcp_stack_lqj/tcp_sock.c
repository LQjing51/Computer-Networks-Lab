...
inline void tcp_set_reno_state(struct tcp_sock *tsk, int state)
{
	log(DEBUG, IP_FMT":%hu switch reno state, from %s to %s.", \
			HOST_IP_FMT_STR(tsk->sk_sip), tsk->sk_sport, \
			tcp_reno_state_str[tsk->reno_state], tcp_reno_state_str[state]);
	tsk->reno_state = state;
}
