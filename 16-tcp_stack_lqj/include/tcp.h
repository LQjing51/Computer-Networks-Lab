...
#define MSS (ETH_FRAME_LEN - (ETHER_HDR_SIZE + IP_BASE_HDR_SIZE + TCP_BASE_HDR_SIZE))
...
// tcp reno states
enum tcp_reno_state { TCP_OPEN, TCP_DISORDER, TCP_RECOVERY, TCP_LOSS };
...
extern const char *tcp_reno_state_str[];
static inline const char *tcp_state_to_str(int state)
{
	return tcp_state_str[state];
}
static inline const char *tcp_reno_state_to_str(int state)
{
	return tcp_reno_state_str[state];
}
