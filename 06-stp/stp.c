...
static bool packet_port_cmp(stp_port_t *p, struct stp_config *config) {
	if (ntohll(config->root_id) < p->designated_root) return true;
	if (ntohll(config->root_id) > p->designated_root) return false;
		
	if (ntohl(config->root_path_cost) < p->designated_cost) return true;
	if (ntohl(config->root_path_cost) > p->designated_cost) return false;

	if (ntohll(config->switch_id) < p->designated_switch) return true;
	if (ntohll(config->switch_id) > p->designated_switch) return false;

	if (ntohs(config->port_id) < p->designated_port) return true;
	if (ntohs(config->port_id) > p->designated_port) return false;

	return false;
}

static bool port_port_cmp(stp_port_t *p, stp_port_t *other) {
	if (other->designated_root < p->designated_root) return true;
	if (other->designated_root > p->designated_root) return false;
		
	if (other->designated_cost < p->designated_cost) return true;
	if (other->designated_cost > p->designated_cost) return false;

	if (other->designated_switch < p->designated_switch) return true;
	if (other->designated_switch > p->designated_switch) return false;

	if (other->designated_port < p->designated_port) return true;
	if (other->designated_port > p->designated_port) return false;

	return false;
}

static void update_current_port_config(stp_port_t *p, struct stp_config *config) {
...
static void update_other_ports_config(stp_t *stp) {
	...
	for (int i = 0; i < stp->nports; i++) {
		...
		if (stp_port_is_designated(p)) {
			...
			// not need to update their designated switch/port, 
			//as they don't receive higher config in their own segments
			p->designated_root = stp->designated_root;
			p->designated_cost = stp->root_path_cost;	
		}else {
			//check and covert undesignated ports to designated ports
			stp_port_t new;
			new.designated_root = stp->designated_root;
			new.designated_cost = stp->root_path_cost;
			new.designated_switch = stp->switch_id;
			new.designated_port = p->port_id;
			if (port_port_cmp(p,&new)) *p = new;
		}
	}
