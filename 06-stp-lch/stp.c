...
// return 1 if received config has higher priority 
static int cmp_recv_config(stp_port_t *p, struct stp_config *config) {
	// compare root id
...
// return 1 if rt has higher priority 
static int cmp_port_config(stp_port_t *p, stp_port_t *rt) {
	// compare root id
	if (rt->designated_root != p->designated_root)
		return rt->designated_root < p->designated_root ? 1 : 0;
	// compare path cost
	if (rt->designated_cost != p->designated_cost)
		return rt->designated_cost < p->designated_cost ? 1 : 0;
	// compare switch id
	if (rt->designated_switch != p->designated_switch)
		return rt->designated_switch < p->designated_switch ? 1 : 0;
	// compare port id
	return rt->designated_port < p->designated_port ? 1 : 0;
}
...
static void stp_update_ports(stp_t *stp) {
	for (int i = 0; i < stp->nports; i++) {
		...
		if (stp_port_is_designated(p)) {
			...
			p->designated_root = stp->designated_root;
			p->designated_cost = stp->root_path_cost;
		} else if (p != stp->root_port) {
			stp_port_t new_port = *p;
			new_port.designated_root = stp->designated_root;
			new_port.designated_cost = stp->root_path_cost;
			new_port.designated_switch = stp->switch_id;
			new_port.designated_port = p->port_id;
			if (cmp_port_config(p, &new_port)) {
				// p becomes designated port
				*p = new_port;
			}
		}
	}
...
static void stp_handle_config_packet(stp_t *stp, stp_port_t *p,
		struct stp_config *config)
{
	// compare received config with mine
	if (cmp_recv_config(p, config)) {
		/* received config is higher */
		// update port info
